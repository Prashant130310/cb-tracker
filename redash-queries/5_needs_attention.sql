-- ============================================================
-- QUERY 5: CB Needs Attention (FULL — ready to paste)
-- ============================================================
-- Name in Redash: "CB Needs Attention"
-- Shows contributors who need follow-up:
--   Disabled, very low activity, high PDR, trending worse, etc.
--
...
SELECT
    ...
    CASE
        ...
        WHEN b.qc_pdr IS NOT NULL AND b.qc_pdr >= 0.4                  THEN 'High PDR (40%+)'
...
WHERE a.disabled = true
   OR (f.active_rate IS NOT NULL AND f.active_rate < 0.15)
   OR (b.qc_pdr IS NOT NULL AND b.qc_pdr >= 0.4)
   OR (c.last_3_pdr IS NOT NULL AND b.qc_pdr IS NOT NULL AND c.last_3_pdr > b.qc_pdr + 0.1)
   OR (e.last_3_qms IS NOT NULL AND d.avg_qms IS NOT NULL AND e.last_3_qms < d.avg_qms - 0.5)
ORDER BY b.qc_pdr DESC NULLS LAST
